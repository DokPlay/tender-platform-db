...
6. запускает и проверяет оба сдаваемых аналитических запроса, а также разбирает последний верхнеуровневый statement, поэтому комментарии, строковые литералы и psql meta-команды не могут подменить требуемую сортировку результата;
7. проверяет отменённые и прекращённые результаты, независимые статусы тендера и лота, имена компаний, точные денежные колонки с копейками в обоих отчётах, нулевые присуждения, равные рейтинги, `NULLS LAST`, последнюю версию ставки, источник отчётного месяца, округление среднего, валюты и границы периодов;
8. проверяет весь набор Unicode White_Space, конечность временных значений, бизнес-значения по умолчанию, итоговое состояние отложенных lifecycle-ограничений (включая смену identity-ключа) и межтабличные сроки ставок и присуждений;
9. воспроизводит конкурентные изменения дат тендера как с присуждением, так и со ставкой, фиксирует ожидание блокировки второй сессией, затем явно освобождает первую сессию и подтверждает сохранение целостности;
