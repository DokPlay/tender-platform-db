...
3. проверяет таблицы, PK, точные определения FK, `RESTRICT`, уникальность, типы и покрытие FK индексами;
...
7. проверяет отменённые закупки, точное ранжирование, разные валюты, равные суммы и обе границы месяца;
...
Сначала материализуется только набор завершённых лотов отчётного шестимесячного периода. Допущенные ставки выбираются и агрегируются с `count(DISTINCT bidder_company_id)` уже для этого ограниченного набора. Благодаря этому версии одной заявки не считаются разными участниками, соединение со ставками не размножает денежные суммы, а отчёт не сканирует всю историю заявок без необходимости.
