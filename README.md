...
3. проверяет таблицы, PK, точные определения FK и рабочих индексов, `RESTRICT`, уникальность, типы и покрытие FK полными индексами;
...
7. проверяет отменённые закупки, точное ранжирование, последнюю версию ставки, нулевую цену, разные валюты, равные суммы и границы месячного и шестимесячного периодов;
8. удаляет только созданный им контейнер.
...
- компактный частичный индекс только по допущенным ставкам для аналитики конкуренции.
...
Сначала материализуется только набор завершённых лотов отчётного шестимесячного периода. Для каждого участника учитывается статус его версии с максимальным `version_no`: участник считается допущенным, только если эта последняя версия имеет статус `admitted`. Кандидаты выбираются частичным индексом по допущенным ставкам, а наличие более новой версии проверяется уникальным индексом `(lot_id, bidder_company_id, version_no)`. Агрегация выполняется на уровне лота, поэтому версии одной заявки не считаются разными участниками и соединение со ставками не размножает денежные суммы.
